...
```{r}
# time interval for all patients
...
bariatric_alc <- b_longer %>% 
  ...
  mutate(diff = time - lag(time)) %>% 
  mutate(diff = ifelse(censored == 0 & is.na(diff), 0, diff)) %>% 
  mutate(diff = ifelse(censored == 1, as.numeric(difftime(enr_last, index, units = c("days"))),
                       diff))
```

```{r set tstart and tstop}
bariatric_alc_wide <- bariatric_alc %>% 
  arrange(pat_id, FROM_DT) %>% 
  group_by(pat_id)
```
