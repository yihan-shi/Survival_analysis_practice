...
bariatric <- bariatric %>%
  mutate(der_age = 2021 - der_yob,
         time = case_when(
           ...
           is.na(FROM_DT) ~ as.numeric(difftime(clm_last, index, units = c("days")))),
...
test <- test %>% 
  ...
  mutate(tstop = ifelse(n == 1, test[n+1,]$diff - 1, 0),
         tstart = 0) %>% 
  select("pat_id", "time", "diff", "n", "tstart", "tstop", "clm_last", "index")
...
test$tstop[dim(test)[1]] <- as.numeric(difftime(test$clm_last[dim(test)[1]], 
...
```{r set tstart and tstop - 2 patients}
# tstart/tstop for an individual
test <-  b_longer %>% 
  filter(pat_id %in% c("0268AAAAAABHWZFP", "0309AAAAAAAAFGLS")) %>%
  group_by(pat_id) %>% 
  mutate(diff = time - lag(time)) %>% 
  mutate(diff = ifelse(censored == 0 & is.na(diff), 0, diff)) 
...
x <- c("pat_id", "time", "diff", "n", "tstart", "tstop", "clm_last", "index")
colnames(output) <- x
...
for(i in 1:length(unique(a_test))){
  ...
  df$tstop[dim(df)[1]] <- as.numeric(difftime(df$clm_last[dim(df)[1]], 
                                                df$index[dim(df)[1]], 
                                                units = c("days")))
...
b_nonalc <- b_longer %>% 
  ...
  filter(censored == 1) %>% 
  mutate(tstart = 0,
         tstop = as.numeric(difftime(clm_last, index, units = c("days"))),
         n = 1) %>% 
  select("pat_id", "n", "tstart", "tstop")
...
```{r AG}
model_AG <- coxph(Surv(tstart, tstop, censored) ~ type, 
                 method = "breslow", 
                 cluster = pat_id,
