...
# mistake here: one patient ID should be associated with multiple FROM_DT

b_outcom_unique <- b_outcom %>% 
  distinct(PAT_ID, FROM_DT, .keep_all = TRUE)

# require(data.table)
# b_outcom_unique <- data.table(b_outcom_unique)
# b_outcom_unique[, Index := 1:.N , by = c("PAT_ID")]
# 
# # Index is the counter
# b_outcom_unique <- b_outcom_unique %>% 
